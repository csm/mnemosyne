...
impl From<cljrs_value::Value> for ClojureValue {
    fn from(v: cljrs_value::Value) -> Self {
        ...
        match v {
            ...
            V::List(ptr)          => Self::List(
                ptr.get().iter().map(|v| Self::from(v.clone())).collect(),
            ),
            V::Vector(ptr)        => Self::Vector(
                ptr.get().iter().map(|v| Self::from(v.clone())).collect(),
            ),
            ...
            V::Set(set_val)       => Self::Set(
                set_val.iter().map(|v| Self::from(v.clone())).collect(),
            ),
            ...
            other                 => Self::Opaque { tag: format!("{other}") },
        }
    }
}
